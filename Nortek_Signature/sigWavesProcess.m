function   [sigWaves,presWaves] = sigWavesProcess(Data,waterdepth,lon,lat)
...
astQualityCutoff = 0;        % [NOTE: I am still unsure about appropriate quality cutoffs for ast/le]
leQualityCutoff = 5500;
despikePhase = false;           % apply phase-space despiking to raw data
despikeMovMed = true;           % apply moving-median despiking
...
maxWavePeriod = 14;             % max wave period allowed during final screening
...
beamWidth = 2.9;
g = 9.81;
Le = 2*depth*tand(beamWidth/2);
fMaxAst = 1./sqrt( 2*pi*Le*2 / g);


% Calculate sampling frequency
ts = median(diff(Data.Burst_MatlabTimeStamp)) * 86400; % sampling period  [sec]
...
minInd = minInd - 1;
% minInd = find( ~isnan(Data.Burst_VelEast(1,:)),1,'last');
...
% Fill NaNs/Infs if there are enough good points:
if sum(isfinite(u))/length(u) >= 0.75
    t = 1:2048;
    gdInd = isfinite(u); u = interp1( t(gdInd),u(gdInd),t);
    gdInd = isfinite(v); v = interp1( t(gdInd),v(gdInd),t);
    gdInd = isfinite(w); w = interp1( t(gdInd),w(gdInd),t);
end
...
elseif despikeMovMed
    u   = filloutliers( u,  'linear','movmedian',128);
    v   = filloutliers( v,  'linear','movmedian',128);
    w   = filloutliers( w,  'linear','movmedian',128);
    ast = filloutliers( ast,'linear','movmedian',128);
    le  = filloutliers( le, 'linear','movmedian',128);
end
...
[ Hs, Tp, Dp, E, f, a1, b1, a2, b2, check] = UVZwaves2(u, v, ast, fs, fMaxAst);
